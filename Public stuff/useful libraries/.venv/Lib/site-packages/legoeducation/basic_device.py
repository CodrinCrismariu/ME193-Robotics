...
class _BasicDevice:
            ...
            for key, fut in registered_keys:
                ...
                if dq is not None:
                    try:
                        ...
                        pass  # Future already consumed by a response — safe to ignore
                    if not dq:
    ...
    def connect(self, device=None, *, device_name=None, device_mac=None, card_color=None, card_serial=None, device_notification_delay = DEFAULT_DEVICE_NOTIFICATION_DELAY):
        ...
        try:
            ...
            # For RPC major >= 2, only major.minor must match (build number is ignored).
            # For RPC major < 2, require an exact match of all three components.
            if RPC_VERSION[0] >= 2:
                version_mismatch = self.version_rpc[:2] != RPC_VERSION[:2]
            else:
                version_mismatch = self.version_rpc != RPC_VERSION
